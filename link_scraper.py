...
def pick_material(page):
    # Pick Nickel
    metals = WebDriverWait(page, timeout=30).until(lambda d: d.find_element(By.ID, 'ctl00_ContentMain_ucMatGroupTree_LODCS1_msTreeViewn3'))
    metals.click()
    non_ferrous = WebDriverWait(page, timeout=30).until(lambda d: d.find_element(By.ID, 'ctl00_ContentMain_ucMatGroupTree_LODCS1_msTreeViewn13'))
    non_ferrous.click()
    nickel = WebDriverWait(page, timeout=30).until(lambda d: d.find_element(By.ID, 'ctl00_ContentMain_ucMatGroupTree_LODCS1_msTreeViewt23'))
    nickel.click()
    return None
...
def start(page):
    ...
    pick_material(page)
    find = WebDriverWait(page, timeout=30).until(lambda d: d.find_element(By.NAME, 'ctl00$ContentMain$btnSubmit'))
    find.click()
    
    results = WebDriverWait(page, timeout=30).until(lambda d: d.find_element(By.ID, 'tblResults'))
